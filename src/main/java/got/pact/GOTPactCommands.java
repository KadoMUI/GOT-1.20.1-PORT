...
public final class GOTPactCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> d) {
        d.register(Commands.literal("pact")
            ...
            .then(Commands.literal("treasury")
                .executes(c -> treasuryInfo(c.getSource()))
                .then(Commands.literal("deposit")
                    .then(Commands.argument("value", IntegerArgumentType.integer(1))
                        .executes(c -> treasuryDeposit(c.getSource(), IntegerArgumentType.getInteger(c, "value"))))
                    .then(Commands.literal("all")
                        .executes(c -> treasuryDepositAll(c.getSource())))))
    ...
    private static int info(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ...
        long treasury = GOTPactEconomyService.balance(player.server, pact.id());
        source.sendSuccess(() -> Component.literal(pact.name() + " | " + pact.size() + " members | Treasury: "
                + treasury
                + " | PvP protection: " + pact.preventPvp()
                + " | Hired FF protection: " + pact.preventHiredFriendlyFire()
                + " | Map: " + pact.showMapLocations()), false);
        return 1;
    }

    private static int treasuryInfo(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = source.getPlayerOrException();
        GOTPact pact = GOTPactService.forPlayer(player.server, player.getUUID()).orElse(null);
        if (pact == null) {
        ...
        GOTPactTreasuryState treasury = GOTPactEconomyService.treasury(player.server, pact.id()).orElse(null);
        long balance = treasury == null ? 0L : treasury.balance();
        long deposits = treasury == null ? 0L : treasury.totalPlayerDeposits();
        long spent = treasury == null ? 0L : treasury.totalSpent();
        source.sendSuccess(() -> Component.literal("[Pact Treasury] " + pact.name()
                + " balance=" + balance
                + " playerDeposits=" + deposits
                + " spent=" + spent), false);
        return 1;
    }

    private static int treasuryDeposit(CommandSourceStack source, int value)
            throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = source.getPlayerOrException();
        GOTPactEconomyService.TransactionResult result = GOTPactEconomyService.deposit(player, value);
        if (!result.success()) {
            source.sendFailure(Component.literal(result.error()));
            return 0;
        }
        source.sendSuccess(() -> Component.literal("Deposited " + result.transferred()
                + " coin value into your Pact treasury. New balance: " + result.treasuryBalance() + "."), false);
        return 1;
    }

    private static int treasuryDepositAll(CommandSourceStack source)
            throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = source.getPlayerOrException();
        GOTPactEconomyService.TransactionResult result = GOTPactEconomyService.depositAll(player);
        if (!result.success()) {
            source.sendFailure(Component.literal(result.error()));
            return 0;
        }
        source.sendSuccess(() -> Component.literal("Deposited all " + result.transferred()
                + " coin value into your Pact treasury. New balance: " + result.treasuryBalance() + "."), false);
        return 1;
    }
}
