...
public class GOTDothrakiNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTDothrakiNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTDothrakiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTDothrakiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTDothrakiNpcEntity.class, EntityDataSerializers.INT);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, DothrakiNpcRole.DOTHRAKI.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == DothrakiNpcRole.Combat.PASSIVE
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        // The final 1.7.10 build populated Dothraki through procedural camps,
        // not a biome spawn list. Structure placement will call prepareForSpawn.
    ...
    public void prepareForSpawn(DothrakiNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomDothraki(random, resolvedFemale)));
    ...
    @Override public String getFactionId() { return "dothraki"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == DothrakiNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == DothrakiNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == DothrakiNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != DothrakiNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public boolean fireImmune() {
        return getRole() == DothrakiNpcRole.DAENERYS_TARGARYEN || super.fireImmune();
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("DothrakiRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("DothrakiChild", isBaby());
        tag.putInt("DothrakiSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, DothrakiNpcRole.byId(tag.getString("DothrakiRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("DothrakiChild"));
        entityData.set(DATA_SKIN, tag.getInt("DothrakiSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTDothrakiNpcLoadouts.createOffers(getRole());
        return offers;
    }
