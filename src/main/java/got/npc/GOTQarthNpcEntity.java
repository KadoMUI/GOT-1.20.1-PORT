...
/** Shared modern entity implementation for the complete Qarth roster. */
public class GOTQarthNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTQarthNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTQarthNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTQarthNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTQarthNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTQarthNpcEntity(EntityType<? extends GOTQarthNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, QarthNpcRole.QARTH_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == QarthNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTQarthNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("qarth")) {
                QarthNpcRole role = random.nextInt(15) < 10
                        ? QarthNpcRole.QARTH_LEVYMAN
                        : QarthNpcRole.QARTH_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(QarthNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTQarthNpcLoadouts.configure(this);
    }

    public QarthNpcRole getRole() { return QarthNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "qarth"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == QarthNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == QarthNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == QarthNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != QarthNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("QarthRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("QarthChild", isBaby());
        tag.putInt("QarthSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, QarthNpcRole.byId(tag.getString("QarthRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("QarthChild"));
        entityData.set(DATA_SKIN, tag.getInt("QarthSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTQarthNpcLoadouts.createOffers(getRole());
        return offers;
    }
