...
/** Shared modern entity implementation for the complete Pentos roster. */
public class GOTPentosNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTPentosNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTPentosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTPentosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTPentosNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTPentosNpcEntity(EntityType<? extends GOTPentosNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, PentosNpcRole.PENTOS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == PentosNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTPentosNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("pentos")) {
                PentosNpcRole role = random.nextInt(15) < 10
                        ? PentosNpcRole.PENTOS_LEVYMAN
                        : PentosNpcRole.PENTOS_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(PentosNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTPentosNpcLoadouts.configure(this);
    }

    public PentosNpcRole getRole() { return PentosNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "pentos"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == PentosNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == PentosNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == PentosNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != PentosNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("PentosRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("PentosChild", isBaby());
        tag.putInt("PentosSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, PentosNpcRole.byId(tag.getString("PentosRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("PentosChild"));
        entityData.set(DATA_SKIN, tag.getInt("PentosSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTPentosNpcLoadouts.createOffers(getRole());
        return offers;
    }
