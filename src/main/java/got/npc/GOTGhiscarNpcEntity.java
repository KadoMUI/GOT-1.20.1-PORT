...
/** Shared modern entity implementation for the complete Ghiscar roster. */
public class GOTGhiscarNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTGhiscarNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTGhiscarNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTGhiscarNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTGhiscarNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTGhiscarNpcEntity(EntityType<? extends GOTGhiscarNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, GhiscarNpcRole.GHISCAR_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == GhiscarNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTGhiscarNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && isGhiscariCity(metadata.id())) {
                GhiscarNpcRole role = naturalRole(metadata.id());
    ...
    public void prepareForSpawn(GhiscarNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomGhiscar(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTGhiscarNpcLoadouts.configure(this);
    }

    public GhiscarNpcRole getRole() { return GhiscarNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "ghiscar"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == GhiscarNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == GhiscarNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == GhiscarNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != GhiscarNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    private boolean isGhiscariCity(String biomeId) {
        return biomeId.equals("ghiscar_meereen") || biomeId.equals("ghiscar_astapor")
                || biomeId.equals("ghiscar_yunkai") || biomeId.equals("ghiscar_new_ghis");
    }

    private GhiscarNpcRole naturalRole(String biomeId) {
        if (biomeId.equals("ghiscar_meereen") && random.nextBoolean()) {
            return GhiscarNpcRole.GHISCAR_HARPY;
        }
        if (biomeId.equals("ghiscar_astapor") && random.nextBoolean()) {
            return GhiscarNpcRole.GHISCAR_UNSULLIED;
        }
        return random.nextInt(15) < 10
                ? GhiscarNpcRole.GHISCAR_LEVYMAN
                : GhiscarNpcRole.GHISCAR_LEVYMAN_ARCHER;
    }
    ...
    private void applyRoleAttributes() {
        double health = switch (getRole()) {
            case GHISCAR_UNSULLIED -> 50.0D;
            case GREY_WORM -> 40.0D;
            default -> getRole().legendary() ? 30.0D : 20.0D;
        };
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("GhiscarRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("GhiscarChild", isBaby());
        tag.putInt("GhiscarSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, GhiscarNpcRole.byId(tag.getString("GhiscarRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("GhiscarChild"));
        entityData.set(DATA_SKIN, tag.getInt("GhiscarSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTGhiscarNpcLoadouts.createOffers(getRole());
        return offers;
    }
