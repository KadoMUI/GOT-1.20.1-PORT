...
/** Shared modern entity implementation for the complete Ironborn roster. */
public class GOTIronbornNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTIronbornNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTIronbornNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTIronbornNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTIronbornNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTIronbornNpcEntity(EntityType<? extends GOTIronbornNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, IronbornNpcRole.IRONBORN_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == IronbornNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTIronbornNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(IronbornNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTIronbornNpcLoadouts.configure(this);
    }

    public IronbornNpcRole getRole() { return IronbornNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "ironborn"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == IronbornNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == IronbornNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == IronbornNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != IronbornNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case DUNSTAN_DRUMM -> spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:red_rain"));
            case HARRAS_HARLAW -> spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:nightfall"));
            case EURON_GREYJOY -> {
                spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:valyrian_chainmail_boots"));
                spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:valyrian_chainmail_leggings"));
                spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:valyrian_chainmail_chestplate"));
                spawnAtLocation(GOTIronbornNpcLoadouts.stack("got:valyrian_chainmail_helmet"));
            }
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("IronbornRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("IronbornChild", isBaby());
        tag.putInt("IronbornSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, IronbornNpcRole.byId(tag.getString("IronbornRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("IronbornChild"));
        entityData.set(DATA_SKIN, tag.getInt("IronbornSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTIronbornNpcLoadouts.createOffers(getRole());
        return offers;
    }
