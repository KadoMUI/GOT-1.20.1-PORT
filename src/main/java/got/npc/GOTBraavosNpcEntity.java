...
/** Shared modern entity implementation for the complete Braavos roster. */
public class GOTBraavosNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTBraavosNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTBraavosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTBraavosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTBraavosNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTBraavosNpcEntity(EntityType<? extends GOTBraavosNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, BraavosNpcRole.BRAAVOS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == BraavosNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTBraavosNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("braavos")) {
                BraavosNpcRole role = random.nextInt(15) < 10
                        ? BraavosNpcRole.BRAAVOS_LEVYMAN
                        : BraavosNpcRole.BRAAVOS_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(BraavosNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTBraavosNpcLoadouts.configure(this);
    }

    public BraavosNpcRole getRole() { return BraavosNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "braavos"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == BraavosNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == BraavosNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == BraavosNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != BraavosNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("BraavosRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("BraavosChild", isBaby());
        tag.putInt("BraavosSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, BraavosNpcRole.byId(tag.getString("BraavosRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("BraavosChild"));
        entityData.set(DATA_SKIN, tag.getInt("BraavosSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTBraavosNpcLoadouts.createOffers(getRole());
        return offers;
    }
