...
/** Shared modern entity implementation for the complete White Walker roster. */
public class GOTWhiteWalkerNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTWhiteWalkerNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTWhiteWalkerNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTWhiteWalkerNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTWhiteWalkerNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTWhiteWalkerNpcEntity(EntityType<? extends GOTWhiteWalkerNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, WhiteWalkerNpcRole.WIGHT.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == WhiteWalkerNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTWhiteWalkerNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().equals("haunted_forest")) {
                ...
                prepareForSpawn(role, null, role == WhiteWalkerNpcRole.WIGHT && random.nextInt(10) == 0,
                        blockPosition(), 24, "");
            }
    ...
    public void prepareForSpawn(WhiteWalkerNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTWhiteWalkerNpcLoadouts.configure(this);
    }

    public WhiteWalkerNpcRole getRole() { return WhiteWalkerNpcRole.byId(entityData.get(DATA_ROLE)); }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != WhiteWalkerNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        if (getRole() == WhiteWalkerNpcRole.WHITE_WALKER && random.nextFloat() < 0.525F) {
            spawnAtLocation(GOTWhiteWalkerNpcLoadouts.stack("got:ice_shard", 1 + random.nextInt(2)));
        }
        if (getRole() == WhiteWalkerNpcRole.NIGHT_KING) {
            spawnAtLocation(GOTWhiteWalkerNpcLoadouts.stack("got:night_king_sword"));
        }
    }
    ...
    private void applyRoleAttributes() {
        ...
        if (getAttribute(Attributes.ARMOR) != null) {
            getAttribute(Attributes.ARMOR).setBaseValue(getRole().giant() ? 10.0D
                    : getRole() == WhiteWalkerNpcRole.WIGHT ? 0.0D : 15.0D);
        }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("WhiteWalkerRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("WhiteWalkerChild", isBaby());
        tag.putInt("WhiteWalkerSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, WhiteWalkerNpcRole.byId(tag.getString("WhiteWalkerRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("WhiteWalkerChild"));
        entityData.set(DATA_SKIN, tag.getInt("WhiteWalkerSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTWhiteWalkerNpcLoadouts.createOffers(getRole());
        return offers;
    }
