...
/** Shared modern entity implementation for the complete Tyrosh roster. */
public class GOTTyroshNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTTyroshNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTTyroshNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTTyroshNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTTyroshNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTTyroshNpcEntity(EntityType<? extends GOTTyroshNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, TyroshNpcRole.TYROSH_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == TyroshNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTTyroshNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("tyrosh")) {
                TyroshNpcRole role = random.nextInt(15) < 10
                        ? TyroshNpcRole.TYROSH_LEVYMAN
                        : TyroshNpcRole.TYROSH_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(TyroshNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTTyroshNpcLoadouts.configure(this);
    }

    public TyroshNpcRole getRole() { return TyroshNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "tyrosh"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == TyroshNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == TyroshNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == TyroshNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != TyroshNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("TyroshRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("TyroshChild", isBaby());
        tag.putInt("TyroshSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, TyroshNpcRole.byId(tag.getString("TyroshRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("TyroshChild"));
        entityData.set(DATA_SKIN, tag.getInt("TyroshSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTTyroshNpcLoadouts.createOffers(getRole());
        return offers;
    }
