...
/** Shared modern entity implementation for the complete Volantis roster. */
public class GOTVolantisNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTVolantisNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTVolantisNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTVolantisNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTVolantisNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTVolantisNpcEntity(EntityType<? extends GOTVolantisNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, VolantisNpcRole.VOLANTIS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == VolantisNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTVolantisNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("volantis")) {
                VolantisNpcRole role = random.nextInt(15) < 10
                        ? VolantisNpcRole.VOLANTIS_LEVYMAN
                        : VolantisNpcRole.VOLANTIS_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(VolantisNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomEssos(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTVolantisNpcLoadouts.configure(this);
    }

    public VolantisNpcRole getRole() { return VolantisNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "volantis"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == VolantisNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == VolantisNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == VolantisNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != VolantisNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("VolantisRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("VolantisChild", isBaby());
        tag.putInt("VolantisSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, VolantisNpcRole.byId(tag.getString("VolantisRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("VolantisChild"));
        entityData.set(DATA_SKIN, tag.getInt("VolantisSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTVolantisNpcLoadouts.createOffers(getRole());
        return offers;
    }
