...
/** Equipment and trade pools translated from the original Myr classes. */
final class GOTMyrNpcLoadouts {
    private GOTMyrNpcLoadouts() {}

    static void configure(GOTMyrNpcEntity npc) {
        MyrNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case MYR_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case MYR_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
            }
            case MYR_LEVYMAN_ARCHER -> {
        ...
        switch (role) {
            case MYR_BAKER, MYR_BARTENDER, MYR_BLACKSMITH,
                    MYR_BREWER, MYR_BUTCHER, MYR_FISHMONGER,
                    MYR_FLORIST, MYR_GOLDSMITH, MYR_LUMBERMAN,
                    MYR_MASON, MYR_MINER, MYR_SLAVER ->
                    npc.setItemSlot(EquipmentSlot.HEAD, stack("got:robes_helmet"));
            default -> { }
        }
        GOTNpcShieldLoadouts.equip(npc, npc.getRole(), "got:myr_shield");
        npc.updateHeldItem();
    }

    static MerchantOffers createOffers(MyrNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:myr_chestplate", 1);
                sell(offers, 12, "got:myr_helmet", 1);
    ...
    private static void myrArmor(GOTMyrNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:myr_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:myr_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:myr_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:myr_helmet"));
    }

    private static ItemStack randomIronWeapon(GOTMyrNpcEntity npc) {
