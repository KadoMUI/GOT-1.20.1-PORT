...
/** Shared modern entity implementation for the complete Riverlands roster. */
public class GOTRiverlandsNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTRiverlandsNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTRiverlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTRiverlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTRiverlandsNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTRiverlandsNpcEntity(EntityType<? extends GOTRiverlandsNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, RiverlandsNpcRole.RIVERLANDS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == RiverlandsNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTRiverlandsNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(RiverlandsNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTRiverlandsNpcLoadouts.configure(this);
    }

    public RiverlandsNpcRole getRole() { return RiverlandsNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "riverlands"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == RiverlandsNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == RiverlandsNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == RiverlandsNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != RiverlandsNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case TYTOS_BLACKWOOD -> spawnAtLocation(GOTRiverlandsNpcLoadouts.stack("got:reminder"));
            case JASON_MALLISTER -> spawnAtLocation(GOTRiverlandsNpcLoadouts.stack("got:tidewings"));
            case JONOS_BRACKEN -> spawnAtLocation(GOTRiverlandsNpcLoadouts.stack("got:indomitable"));
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("RiverlandsRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("RiverlandsChild", isBaby());
        tag.putInt("RiverlandsSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, RiverlandsNpcRole.byId(tag.getString("RiverlandsRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("RiverlandsChild"));
        entityData.set(DATA_SKIN, tag.getInt("RiverlandsSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTRiverlandsNpcLoadouts.createOffers(getRole());
        return offers;
    }
