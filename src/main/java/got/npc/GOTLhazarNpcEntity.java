...
/** Shared modern entity implementation for the complete Lhazar roster. */
public class GOTLhazarNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTLhazarNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTLhazarNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTLhazarNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTLhazarNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTLhazarNpcEntity(EntityType<? extends GOTLhazarNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, LhazarNpcRole.LHAZAR_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == LhazarNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTLhazarNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().equals("lhazar")) {
                int roll = random.nextInt(21);
                LhazarNpcRole role = roll < 10 ? LhazarNpcRole.LHAZAR_LEVYMAN
                        : roll < 15 ? LhazarNpcRole.LHAZAR_LEVYMAN_ARCHER
                        : roll < 19 ? LhazarNpcRole.LHAZAR_SOLDIER
                        : LhazarNpcRole.LHAZAR_SOLDIER_ARCHER;
    ...
    public void prepareForSpawn(LhazarNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomLhazar(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTLhazarNpcLoadouts.configure(this);
    }

    public LhazarNpcRole getRole() { return LhazarNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "lhazar"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == LhazarNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == LhazarNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == LhazarNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != LhazarNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("LhazarRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("LhazarChild", isBaby());
        tag.putInt("LhazarSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, LhazarNpcRole.byId(tag.getString("LhazarRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("LhazarChild"));
        entityData.set(DATA_SKIN, tag.getInt("LhazarSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTLhazarNpcLoadouts.createOffers(getRole());
        return offers;
    }
