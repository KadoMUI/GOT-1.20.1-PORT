...
/** Equipment and trade pools translated from the original Pentos classes. */
final class GOTPentosNpcLoadouts {
    private GOTPentosNpcLoadouts() {}

    static void configure(GOTPentosNpcEntity npc) {
        PentosNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case PENTOS_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case PENTOS_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
            }
            case PENTOS_LEVYMAN_ARCHER -> {
            ...
            case PENTOS_SOLDIER -> {
                ...
                pentosArmor(npc, true);
            }
            case PENTOS_SOLDIER_ARCHER -> {
                ItemStack melee = randomIronWeapon(npc);
                ItemStack bow = stack("got:longbow");
                npc.setWeapons(melee, bow);
                npc.setRangedWeapon(bow);
            ...
            case ILLYRIO_MOPATIS ->
                    npc.setCombatWeapon(stack("got:alloy_steel_dagger"));

            default -> { }
        }
        switch (role) {
            case PENTOS_BAKER, PENTOS_BARTENDER, PENTOS_BLACKSMITH,
                    PENTOS_BREWER, PENTOS_BUTCHER, PENTOS_FARMER,
                    PENTOS_FISHMONGER, PENTOS_FLORIST, PENTOS_GOLDSMITH,
                    PENTOS_LUMBERMAN, PENTOS_MASON, PENTOS_MINER ->
                    npc.setItemSlot(EquipmentSlot.HEAD, stack("got:robes_helmet"));
            default -> { }
        }
        GOTNpcShieldLoadouts.equip(npc, npc.getRole(), "got:pentos_shield");
        npc.updateHeldItem();
    }

    static MerchantOffers createOffers(PentosNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:pentos_chestplate", 1);
                sell(offers, 12, "got:pentos_helmet", 1);
    ...
    private static void pentosArmor(GOTPentosNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:pentos_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:pentos_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:pentos_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:pentos_helmet"));
    }

    private static ItemStack randomIronWeapon(GOTPentosNpcEntity npc) {
