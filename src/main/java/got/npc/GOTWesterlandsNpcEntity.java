...
/** Shared modern entity implementation for the complete Westerlands roster. */
public class GOTWesterlandsNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTWesterlandsNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTWesterlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTWesterlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTWesterlandsNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTWesterlandsNpcEntity(EntityType<? extends GOTWesterlandsNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, WesterlandsNpcRole.WESTERLANDS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(2, new RangedAttackGoal(this, 1.25D, 35, 20.0F) {
            @Override public boolean canUse() {
                return getRole().combat() == WesterlandsNpcRole.Combat.ARCHER && super.canUse();
            }
        });
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == WesterlandsNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTWesterlandsNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("westerlands")) {
                WesterlandsNpcRole role = random.nextInt(15) < 10
                        ? WesterlandsNpcRole.WESTERLANDS_LEVYMAN
                        : WesterlandsNpcRole.WESTERLANDS_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(WesterlandsNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTWesterlandsNpcLoadouts.configure(this);
    }

    public WesterlandsNpcRole getRole() { return WesterlandsNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "westerlands"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == WesterlandsNpcRole.Trade.NONE;
    }
    ...
    public boolean isAimingBow() {
        return getTarget() != null && getRole().combat() == WesterlandsNpcRole.Combat.ARCHER;
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != WesterlandsNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case QUENTEN_BANEFORT -> spawnAtLocation(GOTWesterlandsNpcLoadouts.stack("got:bane"));
            case TYWIN_LANNISTER -> {
                spawnAtLocation(GOTWesterlandsNpcLoadouts.stack("got:tyrion_lannister_brooch"));
                spawnAtLocation(GOTWesterlandsNpcLoadouts.stack("got:widow_wail"));
                spawnAtLocation(GOTWesterlandsNpcLoadouts.stack("got:oathkeeper"));
            }
            default -> { }
        }
    }
    ...
    void updateHeldItem() {
        ItemStack held;
        if (getTarget() != null && getRole().combat() == WesterlandsNpcRole.Combat.ARCHER) {
            held = rangedWeapon;
    ...
    private void applyRoleAttributes() {
        double health = getRole() == WesterlandsNpcRole.GREGOR_CLEGANE
                ? 50.0D : getRole().legendary() ? 30.0D : 20.0D;
        ...
        if (getAttribute(Attributes.ATTACK_DAMAGE) != null) {
            getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(
                    getRole() == WesterlandsNpcRole.GREGOR_CLEGANE ? 6.0D : 3.0D);
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("WesterlandsRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("WesterlandsChild", isBaby());
        tag.putInt("WesterlandsSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, WesterlandsNpcRole.byId(tag.getString("WesterlandsRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("WesterlandsChild"));
        entityData.set(DATA_SKIN, tag.getInt("WesterlandsSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTWesterlandsNpcLoadouts.createOffers(getRole());
        return offers;
    }
