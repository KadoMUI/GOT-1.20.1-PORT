...
/** Shared modern entity implementation for the complete Dragonstone roster. */
public class GOTDragonstoneNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTDragonstoneNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTDragonstoneNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTDragonstoneNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTDragonstoneNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTDragonstoneNpcEntity(EntityType<? extends GOTDragonstoneNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, DragonstoneNpcRole.DRAGONSTONE_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == DragonstoneNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTDragonstoneNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("dragonstone")) {
                DragonstoneNpcRole role = random.nextInt(15) < 10
                        ? DragonstoneNpcRole.DRAGONSTONE_LEVYMAN
                        : DragonstoneNpcRole.DRAGONSTONE_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(DragonstoneNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTDragonstoneNpcLoadouts.configure(this);
    }

    public DragonstoneNpcRole getRole() { return DragonstoneNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "dragonstone"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == DragonstoneNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == DragonstoneNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == DragonstoneNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != DragonstoneNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case ARDRIAN_CELTIGAR -> spawnAtLocation(GOTDragonstoneNpcLoadouts.stack("got:ardrian_celtigar_axe"));
            case STANNIS_BARATHEON -> spawnAtLocation(GOTDragonstoneNpcLoadouts.stack("got:lightbringer"));
            case MONFORD_VELARYON -> spawnAtLocation(GOTDragonstoneNpcLoadouts.stack("got:cutwave"));
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("DragonstoneRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("DragonstoneChild", isBaby());
        tag.putInt("DragonstoneSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, DragonstoneNpcRole.byId(tag.getString("DragonstoneRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("DragonstoneChild"));
        entityData.set(DATA_SKIN, tag.getInt("DragonstoneSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTDragonstoneNpcLoadouts.createOffers(getRole());
        return offers;
    }
