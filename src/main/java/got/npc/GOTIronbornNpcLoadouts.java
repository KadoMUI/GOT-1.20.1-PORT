...
/** Equipment and trade pools translated from the original Ironborn classes. */
final class GOTIronbornNpcLoadouts {
    private GOTIronbornNpcLoadouts() {}

    static void configure(GOTIronbornNpcEntity npc) {
        IronbornNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case IRONBORN_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case IRONBORN_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
                leatherArmor(npc);
            }
            case IRONBORN_LEVYMAN_ARCHER -> {
            ...
            case IRONBORN_SOLDIER -> {
                ItemStack weapon = randomIronWeapon(npc);
                npc.setWeapons(weapon, weapon);
                ironbornArmor(npc, true);
            }
            case IRONBORN_SOLDIER_ARCHER -> {
                ItemStack melee = randomIronWeapon(npc);
                ItemStack bow = stack("got:longbow");
                npc.setWeapons(melee, bow);
                npc.setRangedWeapon(bow);
            ...
            case DUNSTAN_DRUMM -> npc.setCombatWeapon(stack("got:red_rain"));
            case HARRAS_HARLAW -> npc.setCombatWeapon(stack("got:nightfall"));
            case AERON_GREYJOY, BALON_GREYJOY, RODRIK_HARLAW ->
                    npc.setCombatWeapon(stack("got:alloy_steel_dagger"));
            case YARA_GREYJOY -> npc.setCombatWeapon(stack("got:iron_battleaxe"));
            case ERIK_IRONMAKER -> npc.setCombatWeapon(stack("got:alloy_steel_hammer"));
            case VICTARION_GREYJOY -> npc.setCombatWeapon(stack("got:alloy_steel_battleaxe"));
            case EURON_GREYJOY -> {
    ...
    static MerchantOffers createOffers(IronbornNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case PRIEST -> {
                buy(offers, "minecraft:cod", 12, 3);
                buy(offers, "minecraft:kelp", 20, 2);
                sell(offers, 4, "minecraft:book", 2);
                sell(offers, 8, "minecraft:water_bucket", 1);
            }
            default -> { }
        }
    ...
    private static void ironbornArmor(GOTIronbornNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:ironborn_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:ironborn_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:ironborn_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:ironborn_helmet"));
    }

    private static void valyrianArmor(GOTIronbornNpcEntity npc) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:valyrian_chainmail_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:valyrian_chainmail_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:valyrian_chainmail_chestplate"));
        npc.setItemSlot(EquipmentSlot.HEAD, stack("got:valyrian_chainmail_helmet"));
    }

    private static void leatherArmor(GOTIronbornNpcEntity npc) {
    ...
    private static ItemStack randomIronWeapon(GOTIronbornNpcEntity npc) {
