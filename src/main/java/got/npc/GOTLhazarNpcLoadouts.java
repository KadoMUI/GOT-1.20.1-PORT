...
/** Equipment and trade pools translated from the original Lhazar classes. */
final class GOTLhazarNpcLoadouts {
    private GOTLhazarNpcLoadouts() {}

    static void configure(GOTLhazarNpcEntity npc) {
        LhazarNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case LHAZAR_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case LHAZAR_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
                levymanArmor(npc);
            }
            case LHAZAR_LEVYMAN_ARCHER -> {
            ...
            case LHAZAR_SOLDIER -> {
                ...
                lhazarArmor(npc, true);
            }
            case LHAZAR_SOLDIER_ARCHER -> {
                ItemStack melee = randomIronWeapon(npc);
                ItemStack bow = stack("got:longbow");
                npc.setWeapons(melee, bow);
                npc.setRangedWeapon(bow);
        ...
        switch (role) {
            case LHAZAR_BAKER, LHAZAR_BARTENDER, LHAZAR_BLACKSMITH,
                    LHAZAR_BREWER, LHAZAR_BUTCHER, LHAZAR_FARMER,
                    LHAZAR_FISHMONGER, LHAZAR_GOLDSMITH,
                    LHAZAR_LUMBERMAN, LHAZAR_MASON, LHAZAR_MINER ->
    ...
    static MerchantOffers createOffers(LhazarNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:lhazar_chestplate", 1);
                sell(offers, 12, "got:lhazar_helmet", 1);
    ...
    private static void lhazarArmor(GOTLhazarNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:lhazar_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:lhazar_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:lhazar_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:lhazar_helmet"));
    }

    private static void levymanArmor(GOTLhazarNpcEntity npc) {
    ...
    private static ItemStack dyedLeather(Item item, GOTLhazarNpcEntity npc) {
    ...
    private static ItemStack randomIronWeapon(GOTLhazarNpcEntity npc) {
