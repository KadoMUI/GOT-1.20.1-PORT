...
/** Shared modern entity implementation for the complete Arryn roster. */
public class GOTArrynNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTArrynNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTArrynNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTArrynNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTArrynNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTArrynNpcEntity(EntityType<? extends GOTArrynNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, ArrynNpcRole.ARRYN_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == ArrynNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTArrynNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(ArrynNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTArrynNpcLoadouts.configure(this);
    }
    ...
    public boolean isAimingBow() {
        return getTarget() != null && getRole().combat() == ArrynNpcRole.Combat.ARCHER;
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != ArrynNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case LYN_CORBRAY -> spawnAtLocation(GOTArrynNpcLoadouts.stack("got:lady_forlorn"));
            case HARROLD_HARDYNG -> spawnAtLocation(GOTArrynNpcLoadouts.stack("got:honor"));
            case YOHN_ROYCE -> spawnAtLocation(GOTArrynNpcLoadouts.stack("got:lamentation"));
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("ArrynRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("ArrynChild", isBaby());
        tag.putInt("ArrynSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, ArrynNpcRole.byId(tag.getString("ArrynRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("ArrynChild"));
        entityData.set(DATA_SKIN, tag.getInt("ArrynSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTArrynNpcLoadouts.createOffers(getRole());
        return offers;
    }
