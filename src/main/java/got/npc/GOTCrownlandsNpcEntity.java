...
/** Shared modern entity implementation for the complete Crownlands roster. */
public class GOTCrownlandsNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTCrownlandsNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTCrownlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTCrownlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTCrownlandsNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTCrownlandsNpcEntity(EntityType<? extends GOTCrownlandsNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, CrownlandsNpcRole.CROWNLANDS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == CrownlandsNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTCrownlandsNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(CrownlandsNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTCrownlandsNpcLoadouts.configure(this);
    }

    public CrownlandsNpcRole getRole() { return CrownlandsNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() {
        return switch (getRole()) {
            case JAIME_LANNISTER, TYRION_LANNISTER -> "westerlands";
            case YOREN -> "night_watch";
            case BRONN -> "unaligned";
            default -> "crownlands";
        };
    }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == CrownlandsNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == CrownlandsNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == CrownlandsNpcRole.Combat.HYBRID
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("CrownlandsRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("CrownlandsChild", isBaby());
        tag.putInt("CrownlandsSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, CrownlandsNpcRole.byId(tag.getString("CrownlandsRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("CrownlandsChild"));
        entityData.set(DATA_SKIN, tag.getInt("CrownlandsSkin"));
