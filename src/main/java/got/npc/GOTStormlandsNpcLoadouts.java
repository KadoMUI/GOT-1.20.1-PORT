...
/** Equipment and trade pools translated from the original Stormlands classes. */
final class GOTStormlandsNpcLoadouts {
    private GOTStormlandsNpcLoadouts() {}

    static void configure(GOTStormlandsNpcEntity npc) {
        StormlandsNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case STORMLANDS_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case STORMLANDS_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
                leatherArmor(npc);
            }
            case STORMLANDS_LEVYMAN_ARCHER -> {
            ...
            case STORMLANDS_SOLDIER -> {
                ItemStack weapon = randomIronWeapon(npc);
                npc.setWeapons(weapon, weapon);
                stormlandsArmor(npc, true);
            }
            case STORMLANDS_SOLDIER_ARCHER -> {
                ItemStack melee = randomIronWeapon(npc);
                ItemStack bow = stack("got:longbow");
                npc.setWeapons(melee, bow);
                npc.setRangedWeapon(bow);
    ...
    static MerchantOffers createOffers(StormlandsNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:stormlands_chestplate", 1);
                sell(offers, 12, "got:stormlands_helmet", 1);
    ...
    private static void stormlandsArmor(GOTStormlandsNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:stormlands_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:stormlands_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:stormlands_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:stormlands_helmet"));
    }

    private static void leatherArmor(GOTStormlandsNpcEntity npc) {
    ...
    private static ItemStack randomIronWeapon(GOTStormlandsNpcEntity npc) {
