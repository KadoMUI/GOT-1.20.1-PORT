...
/** Shared modern entity implementation for the complete Stormlands roster. */
public class GOTStormlandsNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTStormlandsNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTStormlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTStormlandsNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTStormlandsNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTStormlandsNpcEntity(EntityType<? extends GOTStormlandsNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, StormlandsNpcRole.STORMLANDS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == StormlandsNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTStormlandsNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(StormlandsNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTStormlandsNpcLoadouts.configure(this);
    }

    public StormlandsNpcRole getRole() { return StormlandsNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "stormlands"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == StormlandsNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == StormlandsNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == StormlandsNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != StormlandsNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case SELWYN_TARTH -> spawnAtLocation(GOTStormlandsNpcLoadouts.stack("got:just_maid"));
            case RENLY_BARATHEON -> spawnAtLocation(GOTStormlandsNpcLoadouts.stack("got:blood_of_true_kings"));
            default -> { }
        }
    }
    ...
    private void applyRoleAttributes() {
        double health = getRole() == StormlandsNpcRole.BRIENNE_TARTH
                ? 40.0D : getRole().legendary() ? 30.0D : 20.0D;
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("StormlandsRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("StormlandsChild", isBaby());
        tag.putInt("StormlandsSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, StormlandsNpcRole.byId(tag.getString("StormlandsRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("StormlandsChild"));
        entityData.set(DATA_SKIN, tag.getInt("StormlandsSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTStormlandsNpcLoadouts.createOffers(getRole());
        return offers;
    }
