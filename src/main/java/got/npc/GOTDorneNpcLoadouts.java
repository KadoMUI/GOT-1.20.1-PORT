...
/** Equipment and trade pools translated from the original Dorne classes. */
final class GOTDorneNpcLoadouts {
    private GOTDorneNpcLoadouts() {}

    static void configure(GOTDorneNpcEntity npc) {
        DorneNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case DORNE_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case DORNE_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
                leatherArmor(npc);
            }
            case DORNE_LEVYMAN_ARCHER -> {
            ...
            case GEROLD_DAYNE -> npc.setCombatWeapon(stack("got:darkstar"));
            case BERIC_DAYNE -> npc.setCombatWeapon(stack("got:dawn"));
            case OBERYN_MARTELL -> npc.setCombatWeapon(stack("got:sunspear"));
            case DORAN_MARTELL -> npc.setCombatWeapon(stack("got:walking_stick"));
            case ELLARYA_SAND -> npc.setCombatWeapon(stack("got:alloy_steel_dagger_poisoned"));
            case AREO_HOTAH -> npc.setCombatWeapon(stack("got:alloy_steel_battleaxe"));
            case ARIANNE_MARTELL -> npc.setCombatWeapon(stack("got:alloy_steel_dagger"));
            default -> {
                ItemStack sword = stack("got:alloy_steel_sword");
                npc.setWeapons(sword, sword);
            }
        }
        GOTNpcShieldLoadouts.equip(npc, npc.getRole(), "got:dorne_shield");
        npc.updateHeldItem();
    }

    static MerchantOffers createOffers(DorneNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:dorne_chestplate", 1);
                sell(offers, 12, "got:dorne_helmet", 1);
    ...
    private static void dorneArmor(GOTDorneNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:dorne_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:dorne_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:dorne_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:dorne_helmet"));
    }

    private static void leatherArmor(GOTDorneNpcEntity npc) {
    ...
    private static ItemStack randomIronWeapon(GOTDorneNpcEntity npc) {
