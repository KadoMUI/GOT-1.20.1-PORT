...
/** Shared modern entity implementation for the complete Lys roster. */
public class GOTLysNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTLysNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTLysNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTLysNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTLysNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTLysNpcEntity(EntityType<? extends GOTLysNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, LysNpcRole.LYS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == LysNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTLysNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("lys")) {
                LysNpcRole role = random.nextInt(15) < 10
                        ? LysNpcRole.LYS_LEVYMAN
                        : LysNpcRole.LYS_LEVYMAN_ARCHER;
    ...
    public void prepareForSpawn(LysNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTLysNpcLoadouts.configure(this);
    }

    public LysNpcRole getRole() { return LysNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "lys"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == LysNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == LysNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == LysNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != LysNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        if (getRole() == LysNpcRole.SALLADHOR_SAAN) {
            spawnAtLocation(GOTLysNpcLoadouts.stack("got:truth"));
            return;
        }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("LysRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("LysChild", isBaby());
        tag.putInt("LysSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, LysNpcRole.byId(tag.getString("LysRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("LysChild"));
        entityData.set(DATA_SKIN, tag.getInt("LysSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTLysNpcLoadouts.createOffers(getRole());
        return offers;
    }
