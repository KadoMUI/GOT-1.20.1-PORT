...
/** Shared modern entity implementation for the complete Asshai roster. */
public class GOTAsshaiNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTAsshaiNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTAsshaiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTAsshaiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTAsshaiNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTAsshaiNpcEntity(EntityType<? extends GOTAsshaiNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, AsshaiNpcRole.ASSHAI_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == AsshaiNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTAsshaiNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().equals("shadow_town")) {
                int roll = random.nextInt(14);
                AsshaiNpcRole role = roll < 10 ? AsshaiNpcRole.ASSHAI_WARRIOR
                        : roll < 12 ? AsshaiNpcRole.ASSHAI_SHADOWBINDER
                        : AsshaiNpcRole.ASSHAI_SPHEREBINDER;
    ...
    public void prepareForSpawn(AsshaiNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomAsshai(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTAsshaiNpcLoadouts.configure(this);
    }

    public AsshaiNpcRole getRole() { return AsshaiNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "asshai"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == AsshaiNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == AsshaiNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == AsshaiNpcRole.Combat.HYBRID
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        ...
        double dy = target.getY(0.3333333333333333D) - getEyeY();
        double dz = target.getZ() - getZ();
        if (getRole() == AsshaiNpcRole.ASSHAI_SHADOWBINDER) {
        ...
        if (getRole() == AsshaiNpcRole.ASSHAI_ARCHMAG) {
            LargeFireball bomb = new LargeFireball(level(), this, dx, dy, dz, 1);
            bomb.setPos(getX(), getEyeY() - 0.1D, getZ());
            level().addFreshEntity(bomb);
            playSound(SoundEvents.FIRECHARGE_USE, 1.0F, 0.8F);
        }
    }
    ...
    protected void customServerAiStep() {
        ...
        LivingEntity target = getTarget();
        if (getRole() != AsshaiNpcRole.ASSHAI_SPHEREBINDER || target == null
                || tickCount % 10 != 0 || distanceToSqr(target) > 576.0D) return;
        double dx = getX() - target.getX();
        double dz = getZ() - target.getZ();
        double horizontal = Math.max(1.0D, Math.sqrt(dx * dx + dz * dz));
        target.push(dx / horizontal * 0.35D, 0.08D, dz / horizontal * 0.35D);
    }

    @Override
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != AsshaiNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public boolean fireImmune() {
        return switch (getRole()) {
            case ASSHAI_SHADOWBINDER, ASSHAI_SPHEREBINDER, ASSHAI_ARCHMAG -> true;
            default -> super.fireImmune();
        };
    }
    ...
    private void applyRoleAttributes() {
        double health = getRole() == AsshaiNpcRole.ASSHAI_ARCHMAG ? 50.0D : 20.0D;
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("AsshaiRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("AsshaiChild", isBaby());
        tag.putInt("AsshaiSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, AsshaiNpcRole.byId(tag.getString("AsshaiRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("AsshaiChild"));
        entityData.set(DATA_SKIN, tag.getInt("AsshaiSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTAsshaiNpcLoadouts.createOffers(getRole());
        return offers;
    }
