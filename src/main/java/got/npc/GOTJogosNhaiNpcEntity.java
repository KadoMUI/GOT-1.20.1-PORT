...
/** Shared modern entity implementation for the complete JogosNhai roster. */
public class GOTJogosNhaiNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTJogosNhaiNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTJogosNhaiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTJogosNhaiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTJogosNhaiNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTJogosNhaiNpcEntity(EntityType<? extends GOTJogosNhaiNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, JogosNhaiNpcRole.JOGOS_NHAI_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == JogosNhaiNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTJogosNhaiNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        // Their homeland was settlement-populated, while the legacy Yi-Ti lists
        // carried Jogos Nhai military incursions.
        GOTBiomeMetadata metadata = PlanetosBiomeManager.getMetadata(
                blockPosition().getX(), blockPosition().getZ());
        if ((spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION)
                && metadata != null && metadata.id().startsWith("yi_ti")) {
            JogosNhaiNpcRole role = random.nextInt(15) < 10
                    ? JogosNhaiNpcRole.JOGOS_NHAI_MAN
                    : JogosNhaiNpcRole.JOGOS_NHAI_ARCHER;
            prepareForSpawn(role, null, false, blockPosition(), 24, "");
    ...
    public void prepareForSpawn(JogosNhaiNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomJogosNhai(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTJogosNhaiNpcLoadouts.configure(this);
    }

    public JogosNhaiNpcRole getRole() { return JogosNhaiNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "jogos_nhai"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == JogosNhaiNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == JogosNhaiNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == JogosNhaiNpcRole.Combat.HYBRID
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        ...
        if (getRole() == JogosNhaiNpcRole.JOGOS_NHAI_ARCHER && random.nextBoolean()) {
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != JogosNhaiNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    @Override
    public boolean canFreeze() { return super.canFreeze(); }
    ...
    private void applyRoleAttributes() {
        double health = getRole() == JogosNhaiNpcRole.TUGAR_KHAN ? 50.0D : 20.0D;
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("JogosNhaiRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("JogosNhaiChild", isBaby());
        tag.putInt("JogosNhaiSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, JogosNhaiNpcRole.byId(tag.getString("JogosNhaiRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("JogosNhaiChild"));
        entityData.set(DATA_SKIN, tag.getInt("JogosNhaiSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTJogosNhaiNpcLoadouts.createOffers(getRole());
        return offers;
    }
