...
/** Equipment and trade pools translated from the original Lorath classes. */
final class GOTLorathNpcLoadouts {
    private GOTLorathNpcLoadouts() {}

    static void configure(GOTLorathNpcEntity npc) {
        LorathNpcRole role = npc.getRole();
        npc.clearLoadout();
        switch (role) {
            case LORATH_MAN -> npc.setCombatWeapon(stack("got:iron_dagger"));
            case LORATH_LEVYMAN -> {
                npc.setCombatWeapon(randomIronWeapon(npc));
                levymanArmor(npc);
            }
            case LORATH_LEVYMAN_ARCHER -> {
            ...
            case LORATH_SOLDIER -> {
                ...
                lorathArmor(npc, true);
            }
            case LORATH_SOLDIER_ARCHER -> {
                ItemStack melee = randomIronWeapon(npc);
                ItemStack bow = stack("got:longbow");
                npc.setWeapons(melee, bow);
                npc.setRangedWeapon(bow);
        ...
        switch (role) {
            case LORATH_BAKER, LORATH_BARTENDER, LORATH_BLACKSMITH,
                    LORATH_BREWER, LORATH_BUTCHER, LORATH_FARMER,
                    LORATH_FISHMONGER, LORATH_FLORIST, LORATH_GOLDSMITH,
                    LORATH_LUMBERMAN, LORATH_MASON, LORATH_MINER ->
                    npc.setItemSlot(EquipmentSlot.HEAD, stack("got:robes_helmet"));
            default -> { }
        }
        GOTNpcShieldLoadouts.equip(npc, npc.getRole(), "got:lorath_shield");
        npc.updateHeldItem();
    }

    static MerchantOffers createOffers(LorathNpcRole role) {
        ...
        switch (role.trade()) {
            ...
            case UNITS -> {
                ...
                sell(offers, 16, "got:lorath_chestplate", 1);
                sell(offers, 12, "got:lorath_helmet", 1);
    ...
    private static void lorathArmor(GOTLorathNpcEntity npc, boolean helmet) {
        npc.setItemSlot(EquipmentSlot.FEET, stack("got:lorath_boots"));
        npc.setItemSlot(EquipmentSlot.LEGS, stack("got:lorath_leggings"));
        npc.setItemSlot(EquipmentSlot.CHEST, stack("got:lorath_chestplate"));
        if (helmet) npc.setItemSlot(EquipmentSlot.HEAD, stack("got:lorath_helmet"));
    }

    private static void levymanArmor(GOTLorathNpcEntity npc) {
    ...
    private static ItemStack dyedLeather(Item item, GOTLorathNpcEntity npc) {
    ...
    private static ItemStack randomIronWeapon(GOTLorathNpcEntity npc) {
