...
/** Shared modern entity implementation for the complete Night Watch roster. */
public class GOTNightWatchNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTNightWatchNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTNightWatchNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTNightWatchNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTNightWatchNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTNightWatchNpcEntity(EntityType<? extends GOTNightWatchNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, NightWatchNpcRole.GIFT_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == NightWatchNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTNightWatchNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && (metadata.id().equals("gift_new") || metadata.id().equals("gift_old"))) {
                boolean child = random.nextInt(7) == 0;
                int roll = random.nextInt(10);
                NightWatchNpcRole role = roll < 6 ? NightWatchNpcRole.GIFT_MAN
                        : roll < 9 ? NightWatchNpcRole.GIFT_GUARD : NightWatchNpcRole.GIFT_ARCHER;
    ...
    public void prepareForSpawn(NightWatchNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTNightWatchNpcLoadouts.configure(this);
    }

    public NightWatchNpcRole getRole() { return NightWatchNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "night_watch"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == NightWatchNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == NightWatchNpcRole.Combat.ARCHER) return true;
        return false;
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != NightWatchNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        if (getRole() == NightWatchNpcRole.JEOR_MORMONT) spawnAtLocation(GOTNightWatchNpcLoadouts.stack("got:longclaw"));
        if (getRole() == NightWatchNpcRole.BENJEN_STARK) spawnAtLocation(GOTNightWatchNpcLoadouts.stack("got:valyrian_sword"));
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("NightWatchRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("NightWatchChild", isBaby());
        tag.putInt("NightWatchSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, NightWatchNpcRole.byId(tag.getString("NightWatchRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("NightWatchChild"));
        entityData.set(DATA_SKIN, tag.getInt("NightWatchSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTNightWatchNpcLoadouts.createOffers(getRole());
        return offers;
    }
