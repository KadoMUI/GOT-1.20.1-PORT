...
/** Shared modern entity implementation for the complete YiTi roster. */
public class GOTYiTiNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTYiTiNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTYiTiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTYiTiNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTYiTiNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTYiTiNpcEntity(EntityType<? extends GOTYiTiNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, YiTiNpcRole.YI_TI_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == YiTiNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTYiTiNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("yi_ti")) {
                int roll = random.nextInt(23);
                YiTiNpcRole role = roll < 10 ? YiTiNpcRole.YI_TI_SOLDIER
                        : roll < 15 ? YiTiNpcRole.YI_TI_SOLDIER_CROSSBOWER
                        : roll < 18 ? YiTiNpcRole.YI_TI_SAMURAI
                        : roll < 21 ? YiTiNpcRole.YI_TI_SAMURAI_FLAMETHROWER
                        : YiTiNpcRole.YI_TI_BOMBARDIER;
    ...
    public void prepareForSpawn(YiTiNpcRole role, @Nullable Boolean female, boolean child,
        ...
        setCustomName(Component.literal(role.legendary()
                ? role.displayName()
                : GOTNpcNames.randomYiTi(random, resolvedFemale)));
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTYiTiNpcLoadouts.configure(this);
    }

    public YiTiNpcRole getRole() { return YiTiNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "yi_ti"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == YiTiNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == YiTiNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == YiTiNpcRole.Combat.HYBRID
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        ...
        if (getRole() == YiTiNpcRole.YI_TI_SAMURAI_FLAMETHROWER) {
        ...
        if (getRole() == YiTiNpcRole.YI_TI_BOMBARDIER) {
        ...
        dy = target.getY(0.3333333333333333D) - arrow.getY();
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != YiTiNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    private void applyRoleAttributes() {
        double health = switch (getRole()) {
            case YI_TI_SAMURAI, YI_TI_SAMURAI_FLAMETHROWER -> 30.0D;
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("YiTiRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("YiTiChild", isBaby());
        tag.putInt("YiTiSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, YiTiNpcRole.byId(tag.getString("YiTiRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("YiTiChild"));
        entityData.set(DATA_SKIN, tag.getInt("YiTiSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTYiTiNpcLoadouts.createOffers(getRole());
        return offers;
    }
