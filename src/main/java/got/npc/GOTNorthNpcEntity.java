...
/**
 * Shared modern implementation for all 27 regular and 18 legendary North NPC
 * variants. The role is synced and NBT-persistent, while the original class
 * distinctions are retained as data and behavior switches.
 */
public class GOTNorthNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTNorthNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTNorthNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTNorthNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTNorthNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTNorthNpcEntity(EntityType<? extends GOTNorthNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, NorthNpcRole.NORTH_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(2, new RangedAttackGoal(this, 1.25D, 35, 20.0F) {
            @Override public boolean canUse() {
                NorthNpcRole.Combat combat = getRole().combat();
                return (combat == NorthNpcRole.Combat.ARCHER
                        || combat == NorthNpcRole.Combat.AXE_THROWER) && super.canUse();
            }
        });
        goalSelector.addGoal(2, new MeleeAttackGoal(this, 1.35D, false) {
            @Override public boolean canUse() {
                return getRole().combat() != NorthNpcRole.Combat.ARCHER
                        && getRole().combat() != NorthNpcRole.Combat.AXE_THROWER
                        && super.canUse();
            }
        });
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == NorthNpcRole.Combat.PASSIVE && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTNorthNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(NorthNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTNorthNpcLoadouts.configure(this);
    }

    public NorthNpcRole getRole() { return NorthNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "north"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == NorthNpcRole.Trade.NONE;
    }
    ...
    public EntityDimensions getDimensions(Pose pose) {
        // Legendary size multipliers were renderer-only in 1.7.10.  Only
        // children changed their collision dimensions.
        float scale = isBaby() ? 0.5F : 1.0F;
        return super.getDimensions(pose).scale(scale);
    }

    public boolean isAimingBow() {
        return getTarget() != null && getRole().combat() == NorthNpcRole.Combat.ARCHER;
    }
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        if (getRole().combat() == NorthNpcRole.Combat.AXE_THROWER) {
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != NorthNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case ROBB_STARK -> spawnAtLocation(GOTNorthNpcLoadouts.stack("got:blood_of_true_kings"));
            case WYMAN_MANDERLY -> spawnAtLocation(GOTNorthNpcLoadouts.stack("got:fin"));
            case ARYA_STARK -> spawnAtLocation(GOTNorthNpcLoadouts.stack("got:needle"));
            default -> { }
        }
    }
    ...
    void updateHeldItem() {
        ItemStack held;
        if (getTarget() != null && (getRole().combat() == NorthNpcRole.Combat.ARCHER
                || getRole().combat() == NorthNpcRole.Combat.AXE_THROWER)) {
            held = rangedWeapon;
    ...
    private void applyRoleAttributes() {
        if (getAttribute(Attributes.MAX_HEALTH) != null) {
            double health = getRole() == NorthNpcRole.HODOR ? 40.0D : getRole().legendary() ? 30.0D : 20.0D;
        ...
        if (getAttribute(Attributes.ATTACK_DAMAGE) != null) {
            getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(getRole() == NorthNpcRole.HODOR ? 5.0D : 3.0D);
        }
    }

    private static NorthNpcRole randomHillmanFighter(RandomSource random) {
        int value = random.nextInt(18);
        if (value < 10) return NorthNpcRole.NORTH_HILLMAN_WARRIOR;
        if (value < 15) return NorthNpcRole.NORTH_HILLMAN_ARCHER;
        return NorthNpcRole.NORTH_HILLMAN_AXE_THROWER;
    }

    @Override
    public void addAdditionalSaveData(CompoundTag tag) {
        super.addAdditionalSaveData(tag);
        tag.putString("NorthRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("NorthChild", isBaby());
        tag.putInt("NorthSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, NorthNpcRole.byId(tag.getString("NorthRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("NorthChild"));
        entityData.set(DATA_SKIN, tag.getInt("NorthSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTNorthNpcLoadouts.createOffers(getRole());
        return offers;
    }
