...
/** Shared modern entity implementation for the complete Wildling roster. */
public class GOTWildlingNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTWildlingNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTWildlingNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTWildlingNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTWildlingNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTWildlingNpcEntity(EntityType<? extends GOTWildlingNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, WildlingNpcRole.WILDLING.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == WildlingNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTWildlingNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(WildlingNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTWildlingNpcLoadouts.configure(this);
    }

    public WildlingNpcRole getRole() { return WildlingNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "wildling"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == WildlingNpcRole.Trade.NONE;
    }
    ...
    public boolean isAimingBow() {
        return getTarget() != null && getRole().combat() == WildlingNpcRole.Combat.ARCHER;
    }

    private boolean usesRangedCombat() {
        if (getRole().combat() == WildlingNpcRole.Combat.ARCHER) return true;
        return getRole().combat() == WildlingNpcRole.Combat.AXE_THROWER;
    }
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        if (getRole().combat() == WildlingNpcRole.Combat.AXE_THROWER) {
            GOTThrownAxeEntity axe = new GOTThrownAxeEntity(level(), this);
            double dx = target.getX() - getX();
            double dy = target.getY(0.5D) - axe.getY();
            double dz = target.getZ() - getZ();
            double horizontal = Math.sqrt(dx * dx + dz * dz);
            axe.shoot(dx, dy + horizontal * 0.15D, dz, 1.25F, 9.0F);
            level().addFreshEntity(axe);
            playSound(SoundEvents.SNOWBALL_THROW, 1.0F, 0.8F + random.nextFloat() * 0.4F);
            return;
        }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != WildlingNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        if (getRole() == WildlingNpcRole.GIANT) spawnAtLocation(GOTWildlingNpcLoadouts.stack("got:fur", 10));
    }
    ...
    private void applyRoleAttributes() {
        double health = getRole().giant() ? 100.0D : getRole().legendary() ? 30.0D : 20.0D;
        if (getAttribute(Attributes.MAX_HEALTH) != null) {
            getAttribute(Attributes.MAX_HEALTH).setBaseValue(health);
            setHealth((float)health);
        }
        if (getAttribute(Attributes.ARMOR) != null) {
            getAttribute(Attributes.ARMOR).setBaseValue(getRole().giant() ? 8.0D : getRole().legendary() ? 15.0D : 0.0D);
        }
        if (getAttribute(Attributes.ATTACK_DAMAGE) != null) {
            getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(getRole().giant() ? 10.0D : 3.0D);
        }
    }

    private WildlingNpcRole naturalRole(boolean thenn) {
        int roll = random.nextInt(20);
        if (roll == 0) return WildlingNpcRole.GIANT;
        if (thenn) {
            if (roll < 10) return WildlingNpcRole.THENN;
            if (roll < 16) return WildlingNpcRole.THENN_WARRIOR;
            if (roll < 19) return WildlingNpcRole.THENN_ARCHER;
            return WildlingNpcRole.THENN_AXE_THROWER;
        }
        if (roll < 10) return WildlingNpcRole.WILDLING;
        if (roll < 16) return WildlingNpcRole.WILDLING_WARRIOR;
        if (roll < 19) return WildlingNpcRole.WILDLING_ARCHER;
        return WildlingNpcRole.WILDLING_AXE_THROWER;
    }

    @Override
    public void addAdditionalSaveData(CompoundTag tag) {
        super.addAdditionalSaveData(tag);
        tag.putString("WildlingRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("WildlingChild", isBaby());
        tag.putInt("WildlingSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, WildlingNpcRole.byId(tag.getString("WildlingRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("WildlingChild"));
        entityData.set(DATA_SKIN, tag.getInt("WildlingSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTWildlingNpcLoadouts.createOffers(getRole());
        return offers;
    }
