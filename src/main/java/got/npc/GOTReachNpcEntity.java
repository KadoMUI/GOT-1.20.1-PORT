...
/** Shared modern entity implementation for the complete Reach roster. */
public class GOTReachNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTReachNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTReachNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTReachNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTReachNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTReachNpcEntity(EntityType<? extends GOTReachNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, ReachNpcRole.REACH_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == ReachNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTReachNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(ReachNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTReachNpcLoadouts.configure(this);
    }

    public ReachNpcRole getRole() { return ReachNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() {
        return switch (getRole()) {
            case EBROSE -> "crownlands";
            case LORAS_TYRELL -> "stormlands";
            default -> "reach";
        };
    }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == ReachNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == ReachNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == ReachNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != ReachNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case MORIBALD_CHESTER -> spawnAtLocation(GOTReachNpcLoadouts.stack("got:pruner"));
            case LEYTON_HIGHTOWER -> spawnAtLocation(GOTReachNpcLoadouts.stack("got:vigilance"));
            case RANDYLL_TARLY -> spawnAtLocation(GOTReachNpcLoadouts.stack("got:heartsbane"));
            case QUENN_ROXTON -> spawnAtLocation(GOTReachNpcLoadouts.stack("got:orphan_maker"));
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("ReachRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("ReachChild", isBaby());
        tag.putInt("ReachSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, ReachNpcRole.byId(tag.getString("ReachRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("ReachChild"));
        entityData.set(DATA_SKIN, tag.getInt("ReachSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTReachNpcLoadouts.createOffers(getRole());
        return offers;
    }
