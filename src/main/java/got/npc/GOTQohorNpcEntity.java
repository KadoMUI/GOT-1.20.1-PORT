...
/** Shared modern entity implementation for the complete Qohor roster. */
public class GOTQohorNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTQohorNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTQohorNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTQohorNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTQohorNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTQohorNpcEntity(EntityType<? extends GOTQohorNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, QohorNpcRole.QOHOR_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == QohorNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTQohorNpcEntity.class, 5.0F));
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (spawnType == MobSpawnType.NATURAL || spawnType == MobSpawnType.CHUNK_GENERATION) {
            ...
            if (metadata != null && metadata.id().startsWith("qohor")) {
                int roll = random.nextInt(17);
                QohorNpcRole role = roll < 5 ? QohorNpcRole.QOHOR_LEVYMAN
                        : roll < 7 ? QohorNpcRole.QOHOR_LEVYMAN_ARCHER
                        : QohorNpcRole.QOHOR_UNSULLIED;
    ...
    public void prepareForSpawn(QohorNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTQohorNpcLoadouts.configure(this);
    }

    public QohorNpcRole getRole() { return QohorNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "qohor"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == QohorNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == QohorNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == QohorNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != QohorNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    private void applyRoleAttributes() {
        double health = getRole() == QohorNpcRole.QOHOR_UNSULLIED ? 50.0D : 20.0D;
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("QohorRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("QohorChild", isBaby());
        tag.putInt("QohorSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, QohorNpcRole.byId(tag.getString("QohorRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("QohorChild"));
        entityData.set(DATA_SKIN, tag.getInt("QohorSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTQohorNpcLoadouts.createOffers(getRole());
        return offers;
    }
