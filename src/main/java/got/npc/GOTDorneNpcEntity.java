...
/** Shared modern entity implementation for the complete Dorne roster. */
public class GOTDorneNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTDorneNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTDorneNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTDorneNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTDorneNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTDorneNpcEntity(EntityType<? extends GOTDorneNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, DorneNpcRole.DORNE_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == DorneNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTDorneNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(DorneNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTDorneNpcLoadouts.configure(this);
    }

    public DorneNpcRole getRole() { return DorneNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "dorne"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == DorneNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == DorneNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == DorneNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != DorneNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHit) {
        ...
        switch (getRole()) {
            case GEROLD_DAYNE -> spawnAtLocation(GOTDorneNpcLoadouts.stack("got:darkstar"));
            case BERIC_DAYNE -> spawnAtLocation(GOTDorneNpcLoadouts.stack("got:dawn"));
            case OBERYN_MARTELL -> spawnAtLocation(GOTDorneNpcLoadouts.stack("got:sunspear"));
            default -> { }
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("DorneRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("DorneChild", isBaby());
        tag.putInt("DorneSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, DorneNpcRole.byId(tag.getString("DorneRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("DorneChild"));
        entityData.set(DATA_SKIN, tag.getInt("DorneSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTDorneNpcLoadouts.createOffers(getRole());
        return offers;
    }
