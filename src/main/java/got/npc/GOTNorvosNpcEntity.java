...
/** Shared modern entity implementation for the complete Norvos roster. */
public class GOTNorvosNpcEntity extends PathfinderMob implements net.minecraft.world.entity.monster.RangedAttackMob, Merchant, GOTFactionNpc, GOTQuestGiver {
    private static final EntityDataAccessor<String> DATA_ROLE =
            SynchedEntityData.defineId(GOTNorvosNpcEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> DATA_FEMALE =
            SynchedEntityData.defineId(GOTNorvosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> DATA_CHILD =
            SynchedEntityData.defineId(GOTNorvosNpcEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> DATA_SKIN =
            SynchedEntityData.defineId(GOTNorvosNpcEntity.class, EntityDataSerializers.INT);
    ...
    public GOTNorvosNpcEntity(EntityType<? extends GOTNorvosNpcEntity> type, Level level) {
        super(type, level);
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(DATA_ROLE, NorvosNpcRole.NORVOS_MAN.id());
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(3, new AvoidEntityGoal<>(this, Monster.class, 8.0F, 1.2D, 1.5D) {
            @Override public boolean canUse() {
                return getRole().combat() == NorvosNpcRole.Combat.PASSIVE
                        && getTarget() == null && super.canUse();
            }
        });
        goalSelector.addGoal(5, new RandomStrollGoal(this, 0.9D));
        goalSelector.addGoal(7, new LookAtPlayerGoal(this, Player.class, 8.0F));
        goalSelector.addGoal(8, new LookAtPlayerGoal(this, GOTNorvosNpcEntity.class, 5.0F));
    ...
    public void prepareForSpawn(NorvosNpcRole role, @Nullable Boolean female, boolean child,
        ...
        refreshDimensions();
        applyRoleAttributes();
        if (!level().isClientSide) GOTNorvosNpcLoadouts.configure(this);
    }

    public NorvosNpcRole getRole() { return NorvosNpcRole.byId(entityData.get(DATA_ROLE)); }
    @Override public String getFactionId() { return "norvos"; }
    ...
    @Override public boolean canOfferQuests() {
        return isAlive() && !isBaby() && getRole().trade() == NorvosNpcRole.Trade.NONE;
    }
    ...
    private boolean usesRangedCombat() {
        if (getRole().combat() == NorvosNpcRole.Combat.ARCHER) return true;
        LivingEntity target = getTarget();
        return getRole().combat() == NorvosNpcRole.Combat.HYBRID
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (getRole().trade() != NorvosNpcRole.Trade.NONE && !isBaby() && isAlive()) {
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putString("NorvosRole", getRole().id());
        tag.putBoolean("Female", isFemale());
        tag.putBoolean("NorvosChild", isBaby());
        tag.putInt("NorvosSkin", getSkinIndex());
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        entityData.set(DATA_ROLE, NorvosNpcRole.byId(tag.getString("NorvosRole")).id());
        entityData.set(DATA_FEMALE, tag.getBoolean("Female"));
        entityData.set(DATA_CHILD, tag.getBoolean("NorvosChild"));
        entityData.set(DATA_SKIN, tag.getInt("NorvosSkin"));
    ...
    @Override public MerchantOffers getOffers() {
        if (offers == null) offers = GOTNorvosNpcLoadouts.createOffers(getRole());
        return offers;
    }
