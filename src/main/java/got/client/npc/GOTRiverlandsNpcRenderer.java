...
/** Riverlands skin, legendary overlay and equipment renderer. */
public final class GOTRiverlandsNpcRenderer
        extends HumanoidMobRenderer<GOTRiverlandsNpcEntity, GOTRiverlandsNpcModel> {
    public GOTRiverlandsNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTRiverlandsNpcModel(
                context.bakeLayer(GOTRiverlandsNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTRiverlandsNpcModel(context.bakeLayer(GOTRiverlandsNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTRiverlandsNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTRiverlandsNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTRiverlandsNpcEntity entity) {
        RiverlandsNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTRiverlandsNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTRiverlandsNpcEntity, GOTRiverlandsNpcModel> {
        private final GOTRiverlandsNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTRiverlandsNpcRenderer parent,
                                      GOTRiverlandsNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTRiverlandsNpcEntity entity, float limbSwing, float limbSwingAmount,
