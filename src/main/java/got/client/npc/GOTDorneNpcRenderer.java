...
/** Dorne skin, legendary overlay and equipment renderer. */
public final class GOTDorneNpcRenderer
        extends HumanoidMobRenderer<GOTDorneNpcEntity, GOTDorneNpcModel> {
    public GOTDorneNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTDorneNpcModel(
                context.bakeLayer(GOTDorneNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTDorneNpcModel(context.bakeLayer(GOTDorneNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTDorneNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTDorneNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTDorneNpcEntity entity) {
        DorneNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTDorneNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTDorneNpcEntity, GOTDorneNpcModel> {
        private final GOTDorneNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTDorneNpcRenderer parent,
                                      GOTDorneNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTDorneNpcEntity entity, float limbSwing, float limbSwingAmount,
