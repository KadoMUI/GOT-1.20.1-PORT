...
/** Reach skin, legendary overlay and equipment renderer. */
public final class GOTReachNpcRenderer
        extends HumanoidMobRenderer<GOTReachNpcEntity, GOTReachNpcModel> {
    public GOTReachNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTReachNpcModel(
                context.bakeLayer(GOTReachNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTReachNpcModel(context.bakeLayer(GOTReachNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTReachNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTReachNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTReachNpcEntity entity) {
        ReachNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTReachNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTReachNpcEntity, GOTReachNpcModel> {
        private final GOTReachNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTReachNpcRenderer parent,
                                      GOTReachNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTReachNpcEntity entity, float limbSwing, float limbSwingAmount,
