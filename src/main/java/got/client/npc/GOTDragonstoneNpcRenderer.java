...
/** Dragonstone skin, legendary overlay and equipment renderer. */
public final class GOTDragonstoneNpcRenderer
        extends HumanoidMobRenderer<GOTDragonstoneNpcEntity, GOTDragonstoneNpcModel> {
    public GOTDragonstoneNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTDragonstoneNpcModel(
                context.bakeLayer(GOTDragonstoneNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTDragonstoneNpcModel(context.bakeLayer(GOTDragonstoneNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTDragonstoneNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTDragonstoneNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTDragonstoneNpcEntity entity) {
        DragonstoneNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTDragonstoneNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTDragonstoneNpcEntity, GOTDragonstoneNpcModel> {
        private final GOTDragonstoneNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTDragonstoneNpcRenderer parent,
                                      GOTDragonstoneNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTDragonstoneNpcEntity entity, float limbSwing, float limbSwingAmount,
