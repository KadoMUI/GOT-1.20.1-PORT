...
/** Arryn skin, legendary overlay and equipment renderer. */
public final class GOTArrynNpcRenderer
        extends HumanoidMobRenderer<GOTArrynNpcEntity, GOTArrynNpcModel> {
    public GOTArrynNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTArrynNpcModel(
                context.bakeLayer(GOTArrynNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTArrynNpcModel(context.bakeLayer(GOTArrynNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTArrynNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTArrynNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTArrynNpcEntity entity) {
        ArrynNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTArrynNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTArrynNpcEntity, GOTArrynNpcModel> {
        private final GOTArrynNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTArrynNpcRenderer parent,
                                      GOTArrynNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTArrynNpcEntity entity, float limbSwing, float limbSwingAmount,
