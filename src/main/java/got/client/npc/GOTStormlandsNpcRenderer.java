...
/** Stormlands skin, legendary overlay and equipment renderer. */
public final class GOTStormlandsNpcRenderer
        extends HumanoidMobRenderer<GOTStormlandsNpcEntity, GOTStormlandsNpcModel> {
    public GOTStormlandsNpcRenderer(EntityRendererProvider.Context context) {
        super(context, new GOTStormlandsNpcModel(
                context.bakeLayer(GOTStormlandsNpcLayers.BASE)), 0.5F);
        addLayer(new LegendaryOverlayLayer(this,
                new GOTStormlandsNpcModel(context.bakeLayer(GOTStormlandsNpcLayers.OUTFIT))));
        addLayer(new HumanoidArmorLayer<>(this,
                new HumanoidModel<>(context.bakeLayer(GOTStormlandsNpcLayers.ARMOR_INNER)),
                new HumanoidModel<>(context.bakeLayer(GOTStormlandsNpcLayers.ARMOR_OUTER)),
                context.getModelManager()));
    }

    @Override
    public ResourceLocation getTextureLocation(GOTStormlandsNpcEntity entity) {
        StormlandsNpcRole role = entity.getRole();
        if (role.legendary()) {
    ...
    @Override
    protected void scale(GOTStormlandsNpcEntity entity, PoseStack poseStack, float partialTick) {
        float scale = 0.9375F * entity.getRole().scale();
        poseStack.scale(scale, scale, scale);
    }

    private static final class LegendaryOverlayLayer
            extends RenderLayer<GOTStormlandsNpcEntity, GOTStormlandsNpcModel> {
        private final GOTStormlandsNpcModel outfitModel;

        private LegendaryOverlayLayer(GOTStormlandsNpcRenderer parent,
                                      GOTStormlandsNpcModel outfitModel) {
            super(parent);
        ...
        public void render(PoseStack poseStack, MultiBufferSource buffers, int light,
                           GOTStormlandsNpcEntity entity, float limbSwing, float limbSwingAmount,
