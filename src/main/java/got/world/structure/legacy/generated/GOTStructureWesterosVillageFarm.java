...
public class GOTStructureWesterosVillageFarm extends LegacyNorthernContext {

   protected GOTStructureWesterosVillageFarm(NorthStructureBuilder builder) {
      super(builder, Style.NORTH);
   }


   public static void place(NorthStructureBuilder builder, int variant) {
      GOTStructureWesterosVillageFarm piece = new GOTStructureWesterosVillageFarm(builder);
      ...
      if (this.restrictions) {
         ...
         for (int i1 = -6; i1 <= 6; i1++) {
            for (int k1 = -6; k1 <= 6; k1++) {
               ...
               int k2 = Math.abs(k1);
               int j12;
               if ((i2 == 6 && k2 == 0 || k2 == 6 && i2 == 0)
                  && this.isSurface(world, i1, j12 = this.getTopBlock(world, i1, k1) - 1, k1)
                  && j12 > highestHeight) {
                  highestHeight = j12;
               }
            }
