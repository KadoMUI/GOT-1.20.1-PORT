...
public class GOTStructureNorthHillmanHouse extends LegacyNorthernContext {

   protected GOTStructureNorthHillmanHouse(NorthStructureBuilder builder) {
      super(builder, Style.NORTH);
      this.markerPrefix = "north_hillman";
   }


   public static void place(NorthStructureBuilder builder, int variant) {
      GOTStructureNorthHillmanHouse piece = new GOTStructureNorthHillmanHouse(builder);
   ...
   public boolean generate(LegacyNorthernContext world, Random random, int i, int j, int k, int rotation) {
      ...
      if (this.isTramp) {
         ...
         this.spawnNPCAndSetHome(male, world, 0, 1, 0, 8);
         ...
         this.spawnNPCAndSetHome(female, world, 0, 1, 0, 8);
         LegacyEntity child = new LegacyEntity(world);
         child.getFamilyInfo().setMale(random.nextBoolean());
         child.getFamilyInfo().setChild();
         this.spawnNPCAndSetHome(child, world, 0, 1, 0, 8);
      }

      return true;
   }

   public LegacyNorthernContext setIsTramp() {
      this.isTramp = true;
      return this;
   }
